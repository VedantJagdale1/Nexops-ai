...
| `project:typing` | `{ projectId, isTyping }`                         | none                           | Broadcast an ephemeral typing state to other authorised room members. |
| `chat:send`      | `{ projectId, content, attachments?, mentions? }` | `SocketAck<ProjectMessageDto>` | Validate, persist, and publish a project message.                     |
...
| ------------------ | ------------------------------- | -------------------------------------------------- |
| `project:presence` | `{ projectId, users }`          | Authorised project room.                           |
| `project:typing`   | `{ projectId, user, isTyping }` | Other users in the authorised project room.        |
...
Presence is tracked by user rather than raw socket so multiple browser tabs do not duplicate a person in the roster. It is intentionally ephemeral and resets when the API process restarts.
...
sequenceDiagram
  ...
  Gateway-->>Browser: project:presence
...
- Database persistence completes before `chat:message` is emitted.
